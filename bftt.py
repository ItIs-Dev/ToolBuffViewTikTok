...
import secrets
from thread_pool_utils import ThreadWorkerPool
...
def selec_proxy():
    with open('proxy.txt', 'r', encoding='utf8') as f:
        ...
        if proxy_lines:
            ...
            prxy = None
        return prxy
def mark_result(is_success: bool, reason: str = ''):
    with stats_lock:
        stats['total'] += 1
        if is_success:
            stats['success'] += 1
...
def handle_response(resp: dict):
    # JSON key order không cố định, chỉ check theo nội dung.
    if not isinstance(resp, dict):
        return False, 'json_not_object'

    status_code = resp.get('status_code')
    if status_code != 0:
        return False, f'status_code_{status_code}'

    extra = resp.get('extra')
    log_pb = resp.get('log_pb')

    if not isinstance(extra, dict):
        return False, 'missing_extra'
    if not isinstance(log_pb, dict):
        return False, 'missing_log_pb'
    if 'now' not in extra:
        return False, 'missing_extra_now'
    if 'impr_id' not in log_pb:
        return False, 'missing_log_pb_impr_id'
...
def report_stats():
    ...
    while True:
        ...
        print(
            f"[STATS] total={total} | success={success} | failed={failed} | "
            f"success_rate={success_rate:.2f}% | failed_reasons={reason_text}"
        )
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                mark_result(False, 'json_decode')
                sig = Signature(params='ac=WIFI&op_region=VN', data=str(data), cookies=str(cookie_view)).get_value()
                continue

            is_success, reason = handle_response(resp_json)
            mark_result(is_success, '' if is_success else reason)
            sig = Signature(params='ac=WIFI&op_region=VN', data=str(data), cookies=str(cookie_view)).get_value()
        except requests.Timeout:
            mark_result(False, 'timeout')
            continue
        except requests.RequestException:
            mark_result(False, 'request_exception')
            continue
        except Exception:
            mark_result(False, 'unknown_exception')
            continue

def random_hd(random_hex, sig):
...
def send_view_once():
    ...
    sig = Signature(params='', data='', cookies='').get_value()
    random_hex = secrets.token_hex(16)
    headers_view = random_hd(random_hex, sig)
    cookie_view = {'sessionid': random_hex}
    start = datetime.datetime(2020, 1, 1, 0, 0, 0)
    end = datetime.datetime(2024, 12, 31, 23, 59, 59)
    delta_seconds = int((end - start).total_seconds())
...
threads = []
reporter = threading.Thread(target=report_stats)
...
pool = ThreadWorkerPool(workers=5000, max_queue_size=548557, error_backoff_sec=0.05, name="TikTokBuff")
pool.start()
pool.start_reporter(interval_sec=4.0)
